...
status_cycle = cycle(
    (
        ...
        discord.Activity(
            type=discord.ActivityType.streaming, name=STREAMING_STATUS, url=STREAM_URL
        ),
    )
)
...
async def on_command_error(interaction: discord.Interaction, error):
    ...
    if isinstance(error, app_commands.errors.MissingPermissions):
        msgembed = discord.Embed(
            ...
            description="Looks like you don't have the required permissions to use that command!",
            colour=discord.Color.from_rgb(
                f.error_embed_colour[0],
                f.error_embed_colour[1],
                f.error_embed_colour[2],
            ),
        )
    ...
    if isinstance(error, app_commands.errors.MissingRole):
        msgembed = discord.Embed(
            title="Missing roles",
            description="Looks like you don't have the required roles to use that command!",
            colour=discord.Color.from_rgb(
                f.error_embed_colour[0],
                f.error_embed_colour[1],
                f.error_embed_colour[2],
    ...
    if isinstance(error, app_commands.errors.BotMissingPermissions):
        msgembed = discord.Embed(
            title="Missing permissions",
            description="Looks like I don't have the permissions to use that command!",
            colour=discord.Color.from_rgb(
                f.error_embed_colour[0],
                f.error_embed_colour[1],
                f.error_embed_colour[2],
    ...
    if isinstance(error, app_commands.errors.CommandOnCooldown):
        msgembed = discord.Embed(
            title="Hold on!",
            description=f"Command is on cooldown! Try again after {round(error.retry_after, 2)} seconds.",
            colour=discord.Color.from_rgb(
                f.error_embed_colour[0],
                f.error_embed_colour[1],
                f.error_embed_colour[2],
    ...
    if isinstance(error, app_commands.errors.CommandInvokeError):
        msgembed = discord.Embed(
            title="Command Error!",
            description=f"Internal error happened: {error.original}\n Contact @devboiagru",
            colour=discord.Color.from_rgb(
                f.error_embed_colour[0],
                f.error_embed_colour[1],
                f.error_embed_colour[2],
...
async def help(interaction: discord.Interaction):
    ...
    helpembed = discord.Embed(
        title="Looking for help? I got you 😉",
        description=helptxt,
        colour=discord.Color.from_rgb(
            f.embed_colour[0], f.embed_colour[1], f.embed_colour[2]
        ),
    )
...
async def ai(interaction: discord.Interaction, prompt: str):
    ...
    if len(prompt) > 255:  # Check if prompt is too long
        ...
        if (
            len(result) > 3999
        ):  # If message is huge then slice it into 2 parts and send them seperately
            msgembed = discord.Embed(
                title=prompt,
...
async def unban(interaction: discord.Interaction, memberid: str, reason: str = "None"):
    ...
    try:
        ...
        msgembed = discord.Embed(
            title="Invalid ID inputted",
            description="Required ID is not a valid integer",
            colour=discord.Color.from_rgb(
                f.error_embed_colour[0],
                f.error_embed_colour[1],
                f.error_embed_colour[2],
...
@bot.tree.command(
    name="untimeout",
    description="Removes the timeout from a member (if the member is timed out)",
)
@app_commands.checks.has_permissions(moderate_members=True)
...
async def untimeout(interaction: discord.Interaction, member: discord.Member):
    ...
    msgembed = discord.Embed(
        title=f"Untimed out {member}",
        description="Bro can start yapping again",
        colour=discord.Color.from_rgb(
            f.embed_colour[0], f.embed_colour[1], f.embed_colour[2]
        ),
    )
...
async def nick(
    ...
    msgembed = discord.Embed(
        title=f"Changed {member}'s nickname",
        description=f"New nickname: {nick}",
        colour=discord.Color.from_rgb(
            f.embed_colour[0], f.embed_colour[1], f.embed_colour[2]
        ),
    )
